...
class MysqlSqlGenerator(namingResolver: NamingConventionTransformer?) :
    JdbcSqlGenerator((namingResolver)!!) {
    private val jsonType: DataType<Any>
        get() {
            return JSON_TYPE
        }
    ...
    override fun toDialectType(airbyteProtocolType: AirbyteProtocolType): DataType<*> {
        return when (airbyteProtocolType) {
            ...
            AirbyteProtocolType.TIMESTAMP_WITHOUT_TIMEZONE -> DefaultDataType(
                null,
                LocalDateTime::class.java, "datetime(6)",
            )
        ...
        return columns
            ...
            .map<Field<out Any>> { column: Map.Entry<ColumnId, AirbyteType> ->
                ...
                if (!(isStruct || isArray || (type === AirbyteProtocolType.UNKNOWN))) {
                    ...
                    extractedValue = DSL.function(
                        "JSON_VALUE",
                        String::class.java, extractedValue, DSL.`val`("$"),
                    )
                }
                if (isStruct) {
                    ...
                    return@map when (type) {
                        AirbyteProtocolType.TIME_WITH_TIMEZONE -> DSL.case_()
                            .`when`<Any?>(
                                castedValue.notLikeRegex("^[0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]+)?([-+][0-9]{2}(:?[0-9]{2})?|Z)$"),
                                DSL.`val`<Any?>(null as Any?),
                            )
                            .else_(castedValue)
                            .`as`(quotedName(column.key.name))

                        AirbyteProtocolType.TIMESTAMP_WITH_TIMEZONE -> DSL.case_()
                            .`when`<Any?>(
                                castedValue.notLikeRegex("^[0-9]+-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]+)?([-+][0-9]{2}(:?[0-9]{2})?|Z)$"),
                                DSL.`val`<Any?>(null as Any?),
                            )
                            .else_(castedValue)
                            .`as`(quotedName(column.key.name))
    ...
    override fun castedField(
        ...
        return if (type == AirbyteProtocolType.BOOLEAN) {
            ...
            DSL.cast(field, toDialectType(type))
        }
    }
    ...
    override fun createTable(stream: StreamConfig, suffix: String, force: Boolean): Sql {
        ...
        if (stream.destinationSyncMode === DestinationSyncMode.APPEND_DEDUP) {
            ...
            val indexColumnStream: Stream<Field<*>> = Stream.of(
                ...
                Stream.of<Field<Any>>(DSL.field(name(JavaBaseConstants.COLUMN_NAME_AB_EXTRACTED_AT))),
            ...
            val indexColumns: List<Field<*>> = indexColumnStream.toList()
            statements.add(
                Sql.of(
                    dslContext.createIndex("dedup_idx").on(
                        DSL.table(finalTableName),
                        indexColumns,
                    ).sql,
                ),
            )
        ...
        statements.add(
            Sql.of(
                dslContext.createIndex("extracted_at_idx").on(
                    finalTableName,
        ...
        statements.add(
            Sql.of(
                dslContext.createIndex("raw_id_idx").on(
                    finalTableName,
    ...
    private fun getIndexColumnField(column: ColumnId, airbyteType: AirbyteType): Field<*> {
        ...
        if ("varchar".equals(typeName, ignoreCase = true) || "text".equals(
                typeName,
                ignoreCase = true,
            )) {
            ...
            val colDecl: String =
                dslContext.render(quotedName(column.name)) + "(" + 50 + ")"
            return DSL.field(DSL.sql(colDecl))
        } else {
            return DSL.field(quotedName(column.name))
        }
    }
    ...
    override fun buildAirbyteMetaColumn(columns: LinkedHashMap<ColumnId, AirbyteType>): Field<*> {
        ...
        return DSL.cast(DSL.`val`("{}"), jsonType).`as`(JavaBaseConstants.COLUMN_NAME_AB_META)
    }

    override fun getFinalTableMetaColumns(includeMetaColumn: Boolean): LinkedHashMap<String, DataType<*>> {
        ...
        metaColumns[JavaBaseConstants.COLUMN_NAME_AB_EXTRACTED_AT] = SQLDataType.TIMESTAMP(6).nullable(false)
        return metaColumns
    }

    override fun cdcDeletedAtNotNullCondition(): Condition {
        return DSL.field(name(JavaBaseConstants.COLUMN_NAME_AB_LOADED_AT)).isNotNull()
    ...
    override fun getRowNumber(
        ...
        val primaryKeyFields: List<Field<*>> =
            primaryKey.stream().map { columnId: ColumnId ->
                DSL.field(
        ...
        cursorField.ifPresent { columnId: ColumnId ->
            orderedFields.add(
                DSL.field(quotedName(columnId.name)).desc(),
            )
        }
        orderedFields.add(
            DSL.field(quotedName(JavaBaseConstants.COLUMN_NAME_AB_EXTRACTED_AT)).desc(),
        )
    ...
    //    return dslContext.alterTable(name(stream.finalNamespace, stream.finalName + finalSuffix))
    ...
    //        .collect(LinkedHashMap::new,
    //            (map, column) -> map.put(column.getKey(), jdbcTypeNameFromPostgresTypeName(column.getValue().getType())),
    //            LinkedHashMap::putAll);
    //
    //    final boolean sameColumns = actualColumns.equals(intendedColumns)
    //        && "VARCHAR".equals(existingTable.columns().get(JavaBaseConstants.COLUMN_NAME_AB_RAW_ID).getType())
    //        && "TIMESTAMP".equals(existingTable.columns().get(JavaBaseConstants.COLUMN_NAME_AB_EXTRACTED_AT).getType())
    //        && "JSON".equals(existingTable.columns().get(JavaBaseConstants.COLUMN_NAME_AB_META).getType());
    //
    //    return sameColumns;
    //  }
    override fun migrateFromV1toV2(streamId: StreamId, namespace: String, tableName: String): Sql {
        ...
        return Sql.transactionally(
            ...
            dsl.dropTableIfExists(rawTableName).sql,  // mysql doesn't support `create table (columnDecls...) AS select...`.
            ...
            dsl.createTable(rawTableName)
                .`as`(
                    DSL.select(
                        ...
                        DSL.table(name(namespace, tableName)),
                    ),
                )
    ...
    private fun extractColumnAsJson(column: ColumnId): Field<Any> {
        return DSL.function(
            "JSON_EXTRACT",
            jsonType, DSL.field(name(JavaBaseConstants.COLUMN_NAME_DATA)), jsonPath(column),
        )
    }
    ...
    companion object {
        val JSON_TYPE: DefaultDataType<Any> = DefaultDataType(
            null,
            Any::class.java, "json",
        )
        ...
        private val MYSQL_TYPE_NAME_TO_JDBC_TYPE: Map<String, String> = ImmutableMap.of(
            "text", "clob",
            "bit", "boolean",  // this is atrocious
            "datetime", "datetime(6)",
        )
        ...
        private fun jsonPath(column: ColumnId): Param<String> {
            ...
            val escapedName: String = column.originalName
                .replace("\\", "\\\\")
                .replace("\"", "\\\"")
